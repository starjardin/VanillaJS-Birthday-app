		...
		.then((response) => {
			...
			const generatePeopleList = (people) => {
              ...
              <div class="col-sm btn-container buttons-container">
                <div>
                  ${person.dayLeft < 0
										? `In ${person.dayLeft + 365} days`
										: person.dayLeft === 0 ? 'Today' : person.dayLeft === 1 ? 'Tomorrow' : `In ${person.dayLeft} days`}
                </div>
                <div>
                  <button 
                    type="button" 
          ...
          `
			...
			const filterPeople = () => {
				...
				const searchPerson = people.filter(
					(person) =>
						person.lastName.toLowerCase().trim().includes(inputSearch) ||
